...
public class ManualGenerator(IReadOnlyList<Category> categories, IReadOnlyList<Product> products, IReadOnlyList<ManualToc> manualTocs, IServiceProvider services)
    : SimpleGeneratorBase<Manual>(services)
{
...
Write in markdown format including any headings or subheadings (do NOT wrap the text in a code fence). The total length is around 100 words.
Start your response with the section title, which is at markdown header level 2, and include any relevant subsections.
...
Use markdown formatting, including paragraphs, blank lines, bold, italics, tables, and lists as needed (do NOT use any code fences aka triple backticks).
...
Only output the markdown text for the section (again this section text and any content inside it should NOT be wrapped in a markdown code fence). 
At the end of the section, add the token END_OF_CONTENT.
...
";
            var response = await GetChatCompletion(prompt);
             response = response.Replace("END_OF_CONTENT", "");
